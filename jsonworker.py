...
check_path = r"ExamplePic"  # Папка с файлами(изображениями), откуда мы берём названия файлов
sample = r"https://xn--80acmfijjefoglo2a.xn--90ais/assets/"  # Сэмпл ссылка(sample+file_name+dot)
dot = r'.webp'  # Расширение файла(рекомендуется указывать такой же, как у файла/ов в check_path)
json_file = r"example.json"  # Исходный файл(вписывает имя или путь к обр. файлу .json)
exit_json = r'example-ready.json'  # Готовый файл/Исходный для создания незаконченными изо csv(путь или имя .json)
exit_csv = r'unfilled.csv'  # Готовый файл с незаполненными ссылками в json(файлу задать формат .csv)

################################DISABLED##########################################

color_file = r"colors.json"  # Файл с цветами (отключено) МОЖНО ИГНОРИРОВАТЬ
...
def make_csv():
    with open(exit_json, 'r', encoding="utf-8") as f:
        data = json.load(f)
    # with open(color_file, 'r', encoding="utf-8") as f:  #Работа с цветами (ПРИ НАДОБНОСТИ РАСКОММЕНТИТЬ)
    #     clrs = json.load(f)
    # colors = {}
    # for i in clrs['data']:
